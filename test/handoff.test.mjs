...
process.chdir(tmp);
const { needRequest, readRequested, recentTranscript, buildHandoff } = await import(
  "../index.ts"
);

// --- needRequest -----------------------------------------------------------
assert.equal(needRequest("NEED: package.json, README.md") && 0, 0, "plain NEED");
assert.deepEqual(needRequest("blah\nNEED: a.ts, b.ts\nmore"), ["a.ts", "b.ts"], "mid-text NEED");
...
assert.equal(needRequest("NEED: a, a, a, a, a, a, a, a, a, a").length, 8, "capped at 8");
...
assert.match(readRequested(["one.txt", "sub/two.txt"]), /AAA[\s\S]*BBB/, "two files");
...
fs.writeFileSync(
  ...
  [
    ...
    JSON.stringify({ type: "message", message: { role: "assistant", content: [{ type: "thinking", thinking: "hidden" }, { type: "text", text: "first answer" }] } }),
    JSON.stringify({ type: "message", message: { role: "tool", content: [] } }),
    JSON.stringify({ type: "message", message: { role: "user", content: [{ type: "image", data: "xx", mimeType: "image/png" }, { type: "text", text: "second question" }] } }),
    "not json",
  ].join("\n"),
);
const ctx = {
...
assert.ok(t.includes("first question") && t.includes("second question"), "user turns kept");
...
const q = await buildHandoff(ctx, "the format topic: what do you think?");
assert.ok(q.includes("<<<"), "transcript fenced in");
assert.ok(q.includes("First summarize what matters about: the format topic."), "focus honored");
assert.ok(q.endsWith("what do you think?"), "question appended last");

const q2 = await buildHandoff(ctx, "where are we?");
assert.ok(q2.includes("3-6 bullets"), "no-colon form summarizes generally");
assert.ok(q2.endsWith("where are we?"), "no-colon question last");
...
const none = await buildHandoff(
  { ui: { notify: (m) => seen.push(m), confirm: async () => true }, sessionManager: { getSessionFile: () => null } },
  "q",
);
assert.equal(none, "q", "no transcript falls back to bare question");
assert.ok(seen.some((m) => /no session transcript/.test(m)), "fallback notified");
