...
- **Tool**: `chatgpt_consult` is registered for the LLM. Any session, subagent, or ce-workflow
  ...
  for real in the composer (images, PDFs, text). Images and downloadable files ChatGPT
  generates in answers (code-interpreter results, .md/.json exports) are streamed back the
  same way, saved under `docs/chatgpt/` in the project, and linked from the
  imported/returned answer.
